...
OUTPUT_CSV = OUTPUT_DIR / "sesvetski_kraljevec_705.csv"
OUTPUT_GPKG = OUTPUT_DIR / "sesvetski_kraljevec_705.gpkg"
...
def filter_parcels_with_houses_or_buildings(
    ...
    print("\nHOUSE_ONLY filter:")
    print(f"  parcels before filter: {before_count}")
    ...
    if not building_layers:
        print("  parcels after filter:  0")
        print(f"  parcels removed:       {before_count}")
    ...
    accepted_by_parcel: dict[int, dict] = {}
    ...
    for building_layer in building_layers:
        ...
        for _, joined_row in joined.iterrows():
            ...
            existing = accepted_by_parcel.get(rowid)
            score = (1 if include_hit else 0, float(intersection_area_m2))

            if existing is None or score > existing["score"]:
                accepted_by_parcel[rowid] = {
                    "score": score,
                    "reason": reason,
    ...
    if not result.empty:
        ...
        result["house_filter_reason"] = result["__parcel_rowid"].map(
            lambda rowid: accepted_by_parcel[int(rowid)]["reason"]
        )
        result["house_building_intersection_area_m2"] = result["__parcel_rowid"].map(
            lambda rowid: accepted_by_parcel[int(rowid)]["area"]
        )
        result["house_building_candidate"] = result["__parcel_rowid"].map(
            lambda rowid: accepted_by_parcel[int(rowid)]["candidate"]
        )

    after_count = len(result)

    print(f"  parcels after filter:  {after_count}")
    print(f"  parcels removed:       {before_count - after_count}")
...
def add_google_maps_columns(result: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    ...
    result["google_maps_url"] = result.apply(
        lambda row: f"https://www.google.com/maps?q={row['map_point_lat']},{row['map_point_lon']}",
        axis=1,
    )
